#!/usr/bin/env python3
"""
Generate final 3D PCA projection with Yahya and Lukka and update React app.
"""
...
def main():
    print("📊 Generating final 3D PCA with Yahya and Lukka...")
    
    # Load combined embeddings with Yahya and Lukka
    try:
        with open('book_gemini_embeddings_with_yahya_and_lukka.json', 'r') as f:
            embeddings_data = json.load(f)
    except FileNotFoundError:
        print("❌ Error: book_gemini_embeddings_with_yahya_and_lukka.json not found.")
        return
    
    print(f"📈 Loaded {len(embeddings_data)} user embeddings (including Yahya and Lukka)")
    ...
    # Plot points with Yahya and Lukka highlighted
    for user in users_3d:
        if user['isCurrentUser']:
            ax.scatter(user['x'], user['y'], user['z'], c='red', s=100, alpha=1.0, label='Yahya Rahhawi')
        elif user['isLukka']:
            ax.scatter(user['x'], user['y'], user['z'], c='green', s=100, alpha=1.0, label='Lukka Wolff')
        else:
    ...
    ax.set_title(f"3D PCA: Gemini Book Embeddings with Yahya & Lukka\\n({pca_data['explained_variance']['total']:.1%} total variance explained)")
    ax.legend()
    
    plt.tight_layout()
    plt.savefig('gemini_book_pca_3d_with_yahya_and_lukka.png', dpi=300, bbox_inches='tight')
    
    # Check spread and statistics
    coords_array = np.array([[u['x'], u['y'], u['z']] for u in users_3d])
    
    print(f"\\n🎉 Successfully generated final 3D PCA with Yahya and Lukka!")
    print(f"📁 React data saved to: {react_output}")
    print(f"📁 Embeddings saved to: {react_embeddings}")  
    print(f"📁 Visualization saved to: gemini_book_pca_3d_with_yahya_and_lukka.png")
    ...
    print(f"👤 Yahya Rahhawi marked as current user (red)")
    print(f"👤 Lukka Wolff marked as special user (green)")
